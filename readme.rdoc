...
to make use of hash_subtraction you must first extend the hash instance you wish to call the methods on.
  ...
  # myhash[:a] is the only key value thats the same so it is excluded. The values from myhash2 will be listed if they are different as well as any keys in myhash1 not in myhash2




Copyright (c) 2014Kelly Mahan, released under the MIT license
